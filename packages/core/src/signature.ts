 ...
 *
 * 1. **The return shape varies by host.** The Mini App SDK forwards `sign()` to native
 *    code that is not open source. Two of the strongest builders in the ecosystem shipped
 *    normalisers independently — Reef notes in `src/lib/nimiq/hub.ts` that the SDK
 *    "forwards `sign()` to native code we cannot read, and it may frame differently",
 *    and Cinima ships a `coerceSignBytes` for hex / base64 / `Uint8Array` / `number[]`
 *    (research/verification/04b §B1). chit's entire handshake rests on two devices
 *    producing the same digest, so this is not defensive coding — it is the door.
 *
 * 2. **The result is a RESOLVED union, not only a rejection.** `NimiqProvider.sign()` is
 *    typed `Promise<SignatureResult | ErrorResponse>` and `listAccounts` branches on
 *    `'error' in result` for a resolved value, while the docs say `PermissionDeniedError`
 *    is *thrown* (01 §1.5c). Both paths happen, so both are handled.
 *
 * Byte lengths are the disambiguator, and they are fixed: Ed25519 public keys are 32
 * bytes, signatures are 64. A 64-character string is therefore hex for a public key; a
 * 44-character one is base64 for the same key. Length decides, never a guess.
 */

import { sha256 } from '@noble/hashes/sha256';
import { fromBase64Url } from './hash.ts';
 ...
 * execution). A chit that says "café" or "€40" would fail to verify. Always bytes.
 */
